...
## Purpose
Allow QCHI workflows to use NotebookLM as a knowledge workspace for source-grounded reasoning.

## Usage model
- Use NotebookLM as an external context/retrieval surface for books, papers, and notes.
- Treat NotebookLM outputs as draft evidence that must still pass QCHI quality gates.
...
3. Re-run QCHI derivation + validation checks on retrieved claims.
4. Tag claim provenance and record source mapping.

## Guardrails
- Do not accept NotebookLM output without QCHI verification checks.
